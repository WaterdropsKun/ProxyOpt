...
for idx in range(0, len(noisy_dir_list)):
    ...
    for jdx in range(len(noisy_img_list)):
        ...
        if idx in train_idx:
            noisy_crop_img.save('./SIDD_crop_net/train/NOISY/{}_SRGB/{:03d}.PNG'.format(order, cnt_train))
            gt_crop_img.save('./SIDD_crop_net/train/GT/{}_SRGB/{:03d}.PNG'.format(order, cnt_train))
            ...
            noisy_crop_img.save('./SIDD_crop_net/test/NOISY/{}_SRGB/{:03d}.PNG'.format(order, cnt_train))
            gt_crop_img.save('./SIDD_crop_net/test/GT/{}_SRGB/{:03d}.PNG'.format(order, cnt_train))
